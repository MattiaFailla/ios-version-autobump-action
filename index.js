...
Toolkit.run(async (tools) => {
    ...
    const commitMessage = process.env['INPUT_COMMIT-MESSAGE'] || 'iOS ci: version bump to {{version}}';
    ...
    try {
        ...
        await tools.exec('git', [
            'config',
            'user.email',
            `"${process.env.GITHUB_EMAIL || 'gh-action-bump--ios-version@users.noreply.github.com'}"`,
        ]);
